#!/usr/bin/env node
/**
 * Used as @merrakii/web prestart: fail fast with a clear message if the web port is
 * already taken (common cause of "Internal Server Error" in the browser when
 * a zombie `next-server` is still bound to the web port).
 */
...
await new Promise((resolve, reject) => {
  ...
  server.once("error", (err) => {
    if (err && "code" in err && err.code === "EADDRINUSE") {
      console.error(
        `\n[@merrakii/web] Port ${port} is already in use.\n\n` +
          `From the repo root, free dev/prod ports and restart:\n` +
          `  npm run ports:free\n` +
          `  npm run build -w @merrakii/web && npm run start -w @merrakii/web\n\n` +
